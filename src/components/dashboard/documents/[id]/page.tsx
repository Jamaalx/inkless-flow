...
export default function DocumentSignPage() {
  ...
  const [userSignatures, setUserSignatures] = useState<{ id: string; imageUrl: string }[]>([]);
  ...
  const handleUseExistingSignature = (signatureUrl: string) => {
    if (activeField) {
      setFieldValues({
        ...fieldValues,
        [activeField]: signatureUrl,
      });
      setActiveField(null);
    }
  };
                    ...
                    {userSignatures.length > 0 ? (
                      userSignatures.map((signature) => (
                        <div
                          ...
                          onClick={() => handleUseExistingSignature(signature.imageUrl)}
                        >
                          <div className="bg-white border rounded h-16 w-32 flex items-center justify-center">
                            <img
